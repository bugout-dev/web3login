...
class MoonstreamRegistrationMiddleware(BaseHTTPMiddleware):
    """
    Checks the authorization header on the request. It it represents
    a correctly signer message, adds address attributes to the request.state.
    Otherwise raises a 403 error.
    """
    ...
    async def dispatch(
        ...
        try:
            ...
            json_payload = json.loads(json_payload_str)
            verified = verify(authorization_payload=json_payload, schema="registration")
            address = json_payload.get("address")
            if address is not None:
                address = Web3.toChecksumAddress(address)
            else:
